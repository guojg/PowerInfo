...
function _a6(_a8){
var btn=_a1.nav[_a8];
/*wb 模态窗口弹出页面 点击分页弹出新窗口*/
var a=$("<a onclick=\"void(0)\"></a>").appendTo(tr);
a.wrap("<td></td>");
...
$("<span></span>").attr("href","javascript:void(0)").text(_294).css("margin-left",10).bind("click",eval(_293[_294])).appendTo(tb).linkbutton();
}
}
/*wb重新设置窗口大小时慢*/
win.window({
...
var _29a="<div class=\"messager-icon messager-question\"></div>"+"<div style=\"padding-top:6px;\">"+msg+"</div>"+"<div style=\"clear:both;\"/>";
var _29b={};
...
/*wb tab有虚线去除 */
opts.tab.append("<a href=\"javascript:void(0)\" class=\"tabs-inner\" hidefocus=\"true\" >"+"<span class=\"tabs-title\"></span>"+"<span class=\"tabs-icon\"></span>"+"</a>");
...
if(opts.required){
if(box.val()=="" || box.val().indexOf("请选择")>-1){/*wb*/
box.addClass("validatebox-invalid");
...
/*wb 模态窗口弹出页面 点击combobox弹出新窗口*/
bc.append("<a onclick=\"void(0)\" class=\"textbox-icon "+bb[i].iconCls+"\" icon-index=\""+i+"\"></a>");
}
}
...
var _5a1=$("<div class=\"datagrid-wrap\">"+
				"<div class=\"datagrid-view\">"+
					"<div class=\"datagrid-view1\">"+
					 ...
					 "<div class=\"dr-bar-left\" style=\"display:none;top:0px;width:20px;float:left;\"  />"+	
					 	"<div class=\"datagrid-header\">"+
					 		"<div class=\"datagrid-header-inner\"></div>"+
					 	"</div>"+
						 "<div class=\"dr-bar-right\" style=\"display:none;top:0px;width:20px;float:left;\" />"+
...
/**wb*---->全选不全选的标示*//**wb*----->交叉表checkbox选中问题,解决列选中*/
//var chk_flag = $(this).attr("id");
//if(chk_flag=="quanxuan"){
	//if($(this).is(":checked")){
		//_646(_5ba);
		//}else{
		//_64c(_5ba);
...
/**wb**/
function loadSPBar(target){	
	//判断标题列是否超出显示区域
	var content = $(target).datagrid("getPanel").find("div.datagrid-view2");
	$(content).find("div.dr-bar-right").css({"display":"none"});		
	$(content).find("div.dr-bar-left").css({"display":"none"});	
	var w1 = $(content).find("div.datagrid-header");
	var w2 = $(content).find("div.datagrid-body");	
	var headerRow = $(content).find("table.datagrid-htable").find("tr.datagrid-header-row");
	if(headerRow.children().length==0){return false;}     
    var totalWidth = headerRow[0].clientWidth;
    var bodyOffWidth =  w1[0].clientWidth;   
	if(totalWidth>bodyOffWidth){		 
		var bh = w1[0].clientHeight-3;
		$(content).find("div.dr-bar-right").css({display:"block",height:bh+"px"});		
		$(content).find("div.dr-bar-left").css({display:"none",height:bh+"px"});		
		$(content).find("div.datagrid-body").css({"overflow":"hidden","overflow-y":"auto"});		
		//移动方法
		function scrollByEvent(target,scrollType){
			$(".dr-bar-right").css({display:"block"});
			$(".dr-bar-left").css({display:"block"});
			var target = $(target).datagrid("getPanel").find("div.datagrid-view2");			 
			var ws1 = $(target).find("div.datagrid-header");
			var ws2 = $(target).find("div.datagrid-body");		
			var hr = $(target).find("table.datagrid-htable").find("tr.datagrid-header-row");
		    var bow = ws1[0].offsetWidth;
			var startPos = ws1[0].scrollLeft;		
			var pos = 0;
			var max = hr[0].clientWidth-bow;
			if("R"==scrollType){
				pos = startPos+bow;	 
				if(pos>=max){pos = max;}
				if(pos>=max){
					$(".dr-bar-right").css({display:"none"});
					$(".dr-bar-left").css({display:"block"});
				}
			}else{
				 pos = startPos-bow;
				 ...
				 if(pos-bow<=-bow){
					$(".dr-bar-right").css({display:"block"});
					$(".dr-bar-left").css({display:"none"});
				 }
			}
			ws1.animate({scrollLeft:pos});		
			ws2.animate({scrollLeft:pos});
		}		
		//右边
		$(".dr-bar-right").on("click",{target:target},function(event){	
			scrollByEvent(event.data.target,"R");
		});
		$(".dr-bar-left").on("click",{target:target},function(event){	
			scrollByEvent(event.data.target,"L");
		});		
	}
}
