...
*/
		sb.append("SELECT b.area_id,b.mjz_id jz_id,b.jz_name,concat(b.plant_name,b.jz_name )'dc_jz_name',b.plant_id,b.plant_name,b.code 'unit',b.unit_name ,b.index_y_name,b.code index_y");
		for (String index_x : param.get("index_xs").toString().split(",")) {
			sb.append(",sum(CASE a.index_x WHEN ");
			sb.append(index_x);
			sb.append(" THEN a.value END) '");
		...
		.append("  		SELECT dc.id plant_id,jz.jz_id mjz_id,dc.plant_name,jz.jz_name,jz.area_id ")
		.append("		  FROM (SELECT  area_id,jz_id, MAX(CASE j.index_type WHEN 100 THEN j.index_value END) 'jz_name',")
		.append("			MAX(CASE j.index_type WHEN 200 THEN j.index_value END) 'dc_id' FROM constant_cost_arg j  WHERE j.index_type IN(100,200) and j.area_id=? GROUP BY area_id,jz_id")
		.append(" 				) jz JOIN electricpowerplant_analysis_data dc  	 ON jz.dc_id =dc.id  and jz.area_id=dc.area_id ") ;
		String InSql = "";
		...
		.append("	) b")
		.append("	ON a.jz_id=b.mjz_id AND a.index_y=b.code  and a.area_id=b.area_id and a.area_id=? ") ;
		
		sb.append("	GROUP BY  b.area_id,b.mjz_id,b.code  order by b.mjz_id desc ,index_y asc ") ;
		l.add(index_ys);
		l.add(area_id) ;
